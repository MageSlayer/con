...
ZI esc_on;			/* enable escape sequences? */
...
ZI oes(S x) { P(esc_on,o1s(x)); } /* emit escape seq if enabled */
...
int main(int argc,char *argv[]){
  ...
  esc_on=1;			/* enable escape seq by default */
  S term=getenv("TERM"); if(!term || strcmp(term, "") == 0){ esc_on=0; } /* disable escape seq on empty or missing TERM env var */
  ...
  D=1;P->fd=0;P[1].fd=f;DO(2,P[i].events=POLLIN|POLLHUP);nk();if(T){o1s("\r");oes("\033[0J\0337");rp();}z=r1(ktn(KC,0));while(-1!=poll(P,2,-1))
    if(P[1].revents&POLLIN){if(0>=(c=in(f,4096)))break; jv(&z,kpn(b,c));while((y=gk(&z)))switch((y=d9(y))->t){
      case KC: if(!T)o1k(y);else if(y->n){oes("\0338\033[0J");D=o1k(y);oes("\0337");goto redraw;}}} else if(!(P[0].revents&POLLIN))continue;
    else if(T) switch(c=i0()){
      case 3:case -1:c0();break;
      case 12:redraw:mh();oes("\033[0J");rp();o1(xC+2,xn-2);if(p==xn)break; gotoxy:mh();mr(p/W,p%W);break;
      case 8:case 127:if(p==2)break;if(((p-1)/W)==(p/W))oes("\033[D");else{oes("\033[A");mr(0,W-1);} --p;goto del;//delete
      case 11:xn=p;oes("\033[0J");break;case 4:del:if(p==xn)break;memmove(xC+p,xC+p+1,xn-(p+1));xn--;o1(xC+p,xn-p);o1c(' ');goto gotoxy;case 21:xn=p=2;goto redraw;
      case 1:if(p<W){mr(0,p=2);break;} p=2;goto gotoxy; case 5:p=xn;goto gotoxy;
      case 2:back:if(p==2)break;if(((p-1)/W)==(p/W)){oes("\b");--p;break;} --p;goto gotoxy; case 6:forward:if(p==xn)break;o1c(xC[p]);++p;break;
